...
| `context` | 百分比 + 荧光绿圆环（#39FF14） | 状态栏 tap；无 tap 则该项不画 |
| `cost` | 会话估计花费 `$x.xx` | 状态栏 tap |
| `limits` | `5h 41% · 7d 13%` | 状态栏 tap |
| `message` | §6.4 的标签（+ 正文） | 每个会话自己的状态 |

**上下文占用改成"徽章"：环 + 环心里的数字（2026-09-20，用户报告"横条与状态消息重叠"，见截图）**：`context` 项从 46×7pt 的横条改为**圆环**，读数（整数百分比，**不带 `%`**）画在环心里 —— `baseContextRingDiameter` 17（行高 18，留 1pt 气）、`baseContextRingLineWidth` 2、`baseContextRingFontRatio` 0.62 × 面板自身字号。而**省下的横向空间是这次改动的本意**：默认设置（112pt 宠物、面板 200% = 224pt 宽）下，横条连间隙要 51pt——占整行的约五分之一——而徽章的宽度就是环的直径，读数不再另外要位置（51 → 17pt，约三分之一）。

**数字为什么进环心（2026-09-20 追加，用户问"能不能放进环里的空白、会不会太小"）**：先按"环 + 右边数字"做了一版，然后发现那版**永远不会被画出来**。挤压的规则是"非灵活列在付清缺口前一律降到各自的 `minimumWidth`"（§6.5 的挤压顺序），而面板宽度上限一般只有宠物宽的 200%，行本就是超预算的：环画得起（17pt），"环 + 间隔 + 数字"付不起（按数字用面板自身字号的旁置版算：基准 43pt、1.5× 下约 64pt）。实测：9 项全开、面板宽度从 100% 扫到 300%，那一行画出环的 11 个位置里**带数字的 0 个**（作为对照，只有 3 项的那种宽行 11 个里有 9 个带数字）。数字进环心后，这一项的**声明宽度 = 环直径 = 它的地板**，读数与环同生共死，"有环没数"这个状态从架构上消失了。

**字号的代价是量出来的，不是选的**：环不能高于行（17pt 环在 18pt 行里），2pt 描边后洞只有 13pt（基准）；数字必须塞进洞里，且要按**最宽的读数 "100"** 定字号（字号随位数变会读成 bug）。实测（系统字体，按字号归一）："100" 的墨宽 1.67 倍、墨高 0.74 倍，对角 **1.83** 倍字号 → 13pt 洞装得下 **7.1pt**，即面板 10.5pt 正文的 **0.68**，取 **0.62** 留余量（对角余量 13 − 11.9 ≈ 1pt；四角到洞边约 0.6pt）。也就是说**数字约是同行文字的六成**（用户 1.5× 下约 9.8pt vs 15.75pt）——这是"放进环里"的诚实代价，也正是它换来的东西：唯一一个真的会被画出来的版本。量的是**墨**而不是字体的行盒（数字没有下伸部，行盒多出来的空白落在洞外也没画东西），这条判据由 `MessagePanelLayout.badgeInkFault` 单独给出，`--selftest` 在 80/112/168/224pt 四种宠物上各量一次。

**重叠的根因是"画出来的宽度"与"列拿到的宽度"是两套数**：横条按 `metrics.contextBarSize` 的固定宽度画在 `rect.minX`，而挤压只保证列 ≥ 该列的 `minimumWidth`；当时 `context` 的地板是**通用默认值**"标签最小宽的一半"（17pt，且未乘缩放）——一个和横条毫无关系的数——于是窄面板上列被压到 17pt、横条照旧画满 46pt（大面积下 92pt），直接压在下一列的状态文案上，百分比则被算成 0 宽而消失（用户截图：绿色横条与 "Running" 叠在一起）。所以：

- **项的地板 = 环直径 = 它声明要的宽度**：画多宽就留多宽，低于它整列不画（与 glyph 同一条例外——"13pt 的图标不是被压扁的词"）；读数在环里，所以地板不再有"环够、数不够"的中间态。
- **位置只由 `MessagePanelLayout.usageLayout` 一处决定**：view 照它画，`--selftest` 照它核。圆环水平居中（与 agent glyph 同规矩），按 metrics 的尺寸直接画、**没有夹取**——因为"列的地板不低于环的直径"这条不变式成立时那个夹取永远不生效（混列时地板是该列最宽的那格，可能比环大），而死代码会假装有一条并不存在的防线；真正守住它的是 `--selftest` 对"环必须整只在列内"的断言。这一处也**不量文字**——"要一个环"就是布局期已经做出的声明，而这是在宠物每帧动的时候被问的。
- **数字按墨心居中**：`draw(in:)` 把一行贴它拿到的矩形顶部，而数字没有下伸部，照行盒摆会让它顶到上方的描边上，所以这一行改用 Core Text 的**字形墨界**（`CTLineGetBoundsWithOptions(.useGlyphPathBounds)`）算基线位置，让墨盒中点落在环心（`ink(of:font:)` + `origin(centring:in:)`）。同一条墨界也是"塞不塞得下"的判据（`badgeInkFault`）——绘制与自检量的是同一个东西，不是两套近似。

环的**直径、描边与数字字号乘同一个 `Metrics.scale`**，所以 **Pet → Size** 三者一起放大（112pt 宠物上 17pt 环 / 2pt 描边 / 6.5pt 数字，224pt 上全部翻倍），不会出现"字变大、刻度依旧"的两套比例。`--selftest` 钉五条：三者随 2 倍宠物各自翻倍；面板宽度 100%–300% 全扫一遍，满行（9 项）与宽行（3 项）两种配置下环与数字都不越出所在列；"100" 的墨在 80/112/168/224pt 四种宠物上都塞得进洞（负数对照：把直径换回横条那套固定宽度即 FAIL）；以及徽章确实被画出来（关掉该项会改变渲染）。第五条是混列回归（2026-09-20 第二轮审查后补）：同一列里既有画徽章的行、也有只报 token 数的行时，该列在最小宠物上仍须被画出——地板高于列宽就是整列被丢掉，见上。


**字号、宽度、对齐（2026-09-14 追加，2026-09-16 扩展）**：基准行高 18pt，agent 11.5pt semibold、其余 10.5pt、session 用等宽；面板宽度 = 宠物宽度的百分比（设置里 100–300%，滑杆 + 数值框 + 加减按钮，默认 200%），文字可宽于宠物本体。**对齐是"面板相对宠物"的锚定边，不是行内文字对齐**（2026-09-18 改语义，用户指出）：left = 面板左缘贴宠物左缘向右长，right = 右缘贴右缘向左长，center = 居中（窗口一直以来的行为）；**行内文字永远左对齐**。窗口仍然等于面板，所以精灵不再居中于窗口：`Alignment.spriteOriginX(inWindowWidth:petWidth:)` / `windowOriginX(forSpriteX:...)`（Core，带测试）给出偏移与还原，`PetView.spriteRect`（尾巴、hitTest、追踪区、注视原点都跟着它）、`resizeWindow`、`savePosition` 全用它；存档存的仍是**精灵自己的左缘**，否则一次 left、一次 right 地退出就会把宠物一点点挪走。**整块面板按一个比例缩放**（2026-09-16 起，2026-09-18 改为全面板）：字号设置以"默认 112pt 宠物下的点数"记（默认 10.5pt = 它一直以来的大小，范围 8–20pt），实际大小 = 设置值 × 宠物宽/112，设置界面同时显示换算后的值。**这个比例乘在面板画出的每一个数字上**（`MessagePanelLayout.Metrics`）：四种字体（agent 11.5/其余 10.5/等宽 10.5）、行高 18、行距 2、内边距 9、项间距 8、气泡尾 4、圆角与文字内缩、图标宽 13 与 SF Symbol 点数、上下文徽章 17 / 2 / 0.62×（环直径 / 描边 / 数字比例）、各项宽度上限与 `minimumItemWidth`。默认设置 + 默认宠物时比例恰好是 1，所以**比例这一层**没有任何数字变动：老用户的面板不因缩放而移动（这条不变式是硬要求，改完要用 dump 比像素；2026-09-18 实测 200% 全项、255% 用户配置两种都是 0 像素差异）。**它管的是缩放，不是形状** —— `context` 项从横条换成徽章（2026-09-20）就是有意改变默认呈现的一次，那种改动按用户要求做、并在本节与本文件里留痕，不属于"逐像素不动"的范围。**面板宽度与窗口也乘同一比例**（2026-09-18，用户决定）：字号设置是整块面板的**缩放**，20pt 时同一 255% 面板宽 1.9 倍（286 → 543pt），否则行会被要求在没变宽的盒子里装下放大 1.9 倍的内容——实测就是绿条压到状态措辞上。注意乘的是**设置值相对默认的倍数**（`设置/10.5`），不是 `Metrics.scale`：后者已经含宠物那一份，而面板宽度本来就跟着宠物走，乘两次会让 224pt 宠物的面板变成四倍宽。为什么不是"只跟宠物"（2026-09-16 的原决定）：消息单独放大时，一行里出现两种字号（20pt 的措辞旁边是 10.5pt 的任务名与 7pt 的条），宠物越大元数据相对越小，读起来像 bug 而不是像默认（用户报告，2026-09-18）。**字号调大不能让状态措辞变少**（2026-09-18 修正）：行的挤压从前把消息和其它灵活项一样压到那一档的常数地板（34pt），于是在 112pt 宠物、255% 宽、20pt 时消息只剩 123pt，而"Needs input"加省略号要 127pt——**把字号调大反而比默认字号少看见一截状态词**（用户报告）。现在消息声明自己的地板：标签宽度 + 省略号宽度（正文可以截，标签不行），挤压先动"只会念名字"的项（任务名、工具名）。**这份声明是全有或全无**：行只有在"其余每一项都保住自己的 `minimumWidth` 之后还剩得下整份措辞"时才付，付不起就退回常数地板 34pt——只付一部分等于什么都没买到（措辞照样被截），却让旁边的项白白让出宽度。

**管理器的打开不等检测（2026-09-18，用户报告"开窗有几秒延迟"）**：`openManager` 从前是 `refreshAll()` 之后才 `show()`，而 `refreshAgents()` 在**主线程**上给每个 agent spawn 一次 `--version` —— 本机实测 **6 个 agent 0.57s**（热缓存）/ 冷机更久（第一次从磁盘读那些 CLI），于是窗口要等到整轮检测结束才出现（实测 openManager ≈ 0.96s，其中 refreshAll 572ms + 建窗 387ms）。现在窗口**先**出现（实测 64ms），`refreshAgents()` 把检测放进 `Task.detached` 立即返回，结果回到主 actor 再发布；`isRefreshingAgents` 让 Agents 页显示"Checking what is installed…"而不是空白页，`detectionGeneration` 保证先发的旧一轮不会覆盖新一轮，`onAgentStatusesChanged` 让**菜单栏**（非 SwiftUI）也重建。`--verbose` 打两行：`manager open: window shown in Nms, detection running behind it` 与 `agents: detection took Nms for N`。启动时那次检测同样变成后台（宠物首帧不再等它）。"
...
- **fit 到内容（`fitsText`，默认开）**：面板宽 = clamp(内容宽, 宠物宽, 上限)；上限 = 宽度设置 × 宠物 × 字号倍数（`maximumPanelWidth`），auto 时为默认百分比。内容宽 = Σ列 `fitWidth` + chrome。**消息列的 `fitWidth` = 标签 + 省略号 + min(正文位上限, 实际正文宽)**（`baseMessageDetailAllowance` 120pt × 缩放）：正文最多 200 字，按整条算面板会永远顶在上限、且每条新消息都改一次宽度；按"只算标签"算，正文又永远没地方画（用户报告的两个极端）。**预留必须跟随真实正文**（2026-09-18 追加）：无论有没有正文都留满 120pt，会让一行只写 "Running" 的行在右侧空出一整块，面板看着没铺满（用户报告）——现在没有正文就不预留。**下限是宠物宽**：面板是宠物脚下那块地方，不该比宠物窄。
- **挤压之后剩下的余量摊回列里**（2026-09-18 追加，用户报告："table col 都挤在左侧、右侧有空白"）：`filled` 把余量按各列宽度比例摊回各列（最后一列吃掉取整误差），而不是留在最右或全给某一列——面板比内容宽时（关掉 fit、宠物下限、或内容本来就窄），表格仍要铺满它。列宽仍是共享的，所以每一行的同一项仍从同一 x 开始。`--selftest` 同时钉两条：整行正好等于面板宽（spare ≤ 0.5），且余量确实摊进了列里（"82pt of column for a 13pt glyph"）。
...
- **管理器窗口的尺寸**（2026-09-18，用户报告）：默认 **1620×1224**（原 900×680 的 180%，用户 2026-09-18 说默认不够大；900×680 本身是因为 860×580 太矮——Pets 页是"240pt 预览 + 轨道选择 + 名称描述 + 信息网格 + 按钮"一长条，`Use on Desktop`/`Reveal in Finder` 落在折叠线以下，窗口看起来却没有截断的迹象）、最小仍是 **760×520**、并**记住用户拖出来的尺寸**（`manager.window.size`，存 content 尺寸，和宠物位置一样进 UserDefaults）。尺寸在 **`windowDidResize` 里就写**，不能只在 `windowWillClose`：管理器开着被退出时不会触发 close。
  - **重开窗口必须保住尺寸**（2026-09-18 修）：每次打开都会换掉 content view controller（那是让 SwiftUI 重新投递更新的手段，§6.5c），而 AppKit 在赋值那一刻就把窗口缩到新视图的 fitting size（SwiftUI 的最小值 760×520）——`windowDidResize` 忠实地把**这个中间尺寸**记了下来，于是用户调过大小的窗口再打开就变回最小，而且**下一个 launch 也从最小开**（实测：resize 到 812×596、关、重开 → 760×520 且存成 `760.0,520.0`）。现在重开时先取 frame、换完视图再 `setFrame` 放回去，`isRebuilding` 让那次中间 resize 不进 UserDefaults。`--selftest` 钉住：`show()` 两次之后窗口仍是 812×596、存的也仍是 `812.0,596.0`。
  - **开窗不许大过屏幕**：默认 1620×1224 在 13 吋（1440×900 可见区更小）上比屏幕还大，居中的话四边都露不出去、右下角的缩放角也够不着。`openingContentSize(stored:available:)`（纯函数）把结果同时夹在 **≥ 最小** 和 **≤ 屏幕可见区减去窗口自身 chrome** 之间；chrome 用 `window.contentRect(forFrameRect:)` 量，免得标题栏挂着屏幕外面。挤到多显示器上：只有一份尺寸，被切过就以切过的那次为准。
...
**"不在屏上"这个判据与诊断运行**：`--selftest` 等诊断运行从不把管理器上屏（上游同日加的 `presents = !HeadlessMode.isActive` 守卫，为的是不把用户从正在打字的地方拽走），于是"在屏上"在那里恒为假。`isOpen` 因此把 `HeadlessMode` 也算作开着：那里没有屏幕要维持，重建只会让自检去量一个用户永远遇不到的重建——实测不这样写，`checkSettingsShortcut` 的第二次按键会走开窗路径而报"重建了视图树"。**但"关过的窗口"不算开着**（2026-09-20，合并到 main 时补）：`isVisible` 对"关掉过"与"诊断运行里从没上过屏"给的是同一个答案，而这两件事根本不是一回事——前者已经收不到 SwiftUI 的更新、回来时必须换树，后者没有东西要救活。所以控制器另有 `isClosed`（`windowWillClose` 置位、`show()` 复位），`isOpen` 读它。没有这一条，诊断运行里就永远走不到"关闭 → 重开"这条用户真实路径，`--selftest` 里那条"重开后窗口尺寸还在"（2026-09-18 的窗口尺寸修复）就会在 headless 下**空转着变绿**；同一条判据也让那条断言可以先 `close()` 再 `show()`，去量真正的重建。同时自检找窗口改用**同一性**而不是"按标题找可见窗口"：`close()` 过的管理器窗口（`isReleasedWhenClosed = false`）仍在 `NSApp.windows` 里，而诊断运行里没有任何窗口是可见的，两件事叠加会让标题查找认错窗口。
