...
第四列里的两个 Agent 是同一个家族：Pi 与 Oh My Pi 都是进程内 TS 扩展（后者是前者的重命名分支，扩展 API 相同，只有配置目录不同），也都由本项目的配置器写入各自目录下的一个文件（`~/.pi/agent/extensions/agentpet.ts`、`~/.omp/agent/extensions/agentpet.ts`），安装即写文件、移除即删文件。两者真正的差别是**事件集不同**（Pi 用 `agent_settled` + `ui_prompt_start`，omp 用 `agent_end`(−`willContinue`) + 审批事件），因此生成文件由同一个模板加事件集与所有权标记等显式参数产出，见 §8.8。
...
| 移除 | 只在文件仍含**记录里那行** `// shim:` 时才删（判据是那一行，不是"文件没被人动过"：用户在其上追加内容并不会让它留下）。删除前先备份（同一个备份目录与命名规则，§8.2），所以连用户追加的部分也能取回。那行被改掉/删掉，或记录为空（后者仅 omp：Pi 只认文件头的标记行，不看记录）→ 不删，报 `didChange = false` |
