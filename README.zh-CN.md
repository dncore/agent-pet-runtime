...
工作与等待是"持续状态"：整行一直循环，直到状态结束——三秒就停下的宠物在十分钟的任务里看起来像睡着了。完成与失败是"一次性时刻"：按 Codex 的播放形状整行播三遍，然后沉进呼吸，这也是它不会一直僵在某个表情上的原因。系统要求 reduced motion 时，每个动画定格在自己选中的第 0 帧——注视姿态除外，它本来就只有一帧，"定格"不该变成"看向错的方向"。
...
| 3. Agent 状态 | 上面那张表 |
| 4. 注视 | 折进 `idle` 一行里——十六个姿态，间隔 22.5° |
| 5. 发呆 | 兜底 |

拖动压倒一切：**你手上拿着它**；指针落到宠物身上也一样——Codex 的 hover 是跳**三遍**然后接着呼吸，光标不移开就一直动（"三遍 + idle 段无限循环"是它每个状态共用的形状）。

注视不是独立一层，而是**折进别的行里的**——这正是 Codex 自己 sprite 的写法：look frame 直接顶掉那一行的播放。Codex 给三行发 look frame（`idle`/`running`/`waving`），本项目**只给 `idle`**：注视姿态是**单帧静态**，被顶掉的那一行内容就整个消失。顶 `running` 时用的又是同一个格子，于是有 look 行的宠物干活时和发呆时画出来一模一样——"正在干活"这个状态反而看不见；顶 `waving` 时，宠物的"你好"被画成一次凝视。所以这两处不跟 Codex。等待输入的宠物继续等你，工作中的宠物继续跑，宠物问候你时真的会挥手；死区只有一个点：空闲的宠物看着指针，只有指针正压在它中心时才回到自己的呼吸。
...
这一块现在是一个**会话面板**：每个开着的会话一行，同一 agent 的会话排在一起，宠物正在展示的那个排最前。每行可显示：agent 名、session id 后六位、会话名或项目名、模型（含 effort）、当前工具、上下文占用（荧光绿圆环）、会话花费、5 小时 / 7 天额度用量、状态消息。每一项都可以在管理器的 Settings 里开关与排序；面板可设为常驻或只在有事发生时出现；宽度是宠物宽度的**上限**（宠物宽度的 100–300%，滑杆 + 数值框 + 加减按钮）：面板默认**贴合内容**——所有行共用同一组列（同一项在同一 x，缺项留空），因此看起来是一张对齐的表；它不会超过上限，也不会比宠物窄。面板的字号也可调（默认宠物下 8–20pt）：整块面板——状态措辞、会话名、工具、图标、荧光圆环、行高、内边距——都按同一比例缩放，面板宽度也随之变宽，所以这个设置是**整块面板的缩放**；打开"随宠物缩放"则由宠物尺寸决定这两项，两个滑杆随之置灰。对齐选的是**面板相对宠物的锚定边**（左缘貼左缘 / 居中 / 右缘貼右缘），行内文字永远左对齐。**宠物本身不动**：面板向上生长、并向锚定的一侧展开，宽度涨得即时、缩得等内容稳定下来。状态消息的第二行仍是事件真正携带的信息：在等哪个工具、什么失败了，以及回合结束时**助手最后一条消息的预览**——Claude Code 通过 `last_assistant_message` 把它交给 hook，Codex 也是这么展示的。它按 Codex 的规则整理（折叠空白、截到 200 字符），只存在于内存，绝不写入任何日志文件；你输入过的内容永远不上屏。
...
看起来像宠物、却播放不了的文件夹（版本标记与图纸尺寸矛盾、字段类型写错、spritesheet 不是图像）**会被列出来而不是丢掉**：管理器里有一栏 "Not listed" 写着原因，`--diagnose` 打印同一份内容，一只可用的宠物都没有时启动提示会点名是哪几个文件夹被拒——而不是说"那个目录是空的"。V1/V2 由**实测图纸尺寸**决定（1536×1872 / 1536×2288），manifest 的声明只用来交叉校验。
...
同一条规矩还有更隐蔽的一半：**shim 说出的话**。Antigravity 的 `PreToolUse` 要求钩子的结果里必须带 `decision`，观察者根本无法作答，而 shim 在那里回的 `{}` 被读成了"拒绝"，于是每一次工具调用都被判死（2026-09-17 修复，该事件不再注册）。**结果可能拦下工具调用的钩子，这个运行时一个都不挂**——哪怕空答案在测试里看着毫无副作用。
...
| 悬浮宠物、拖动、注视、位置记忆 | 完成 |
| 宠物尺寸——Codex 自己的 80–224 滑杆，默认 112 | 完成——管理器设置里调 |
